&НаКлиенте
Процедура ПрочитатьЛистExcelПромСвязьБанк(ТЗ = Неопределено, ЛистЭксель = Неопределено, НомерПервойСтроки = 1, НомерПервойКолонки = 1, ВсегоСтрок = 0, ВсегоКолонок = 0) Экспорт	
	...
	Пока Сч<=ФайлСтрок Цикл
		Если ЗначениеЗаполнено(СокрЛП(СтрЗаменить(СокрЛП(Excel.Cells(Сч, 6).Value),Символы.НПП,""))) Тогда
		...
		//НоваяСтр.ДатаПлатежа = СокрЛП(СтрЗаменить(СокрЛП(Excel.Cells(Сч, 11).Value),Символы.НПП,""));
		//НоваяСтр.НомерПлатежа = СокрЛП(СтрЗаменить(СокрЛП(Excel.Cells(Сч, 3).Value),Символы.НПП,""));
		...
		НоваяСтр.НомерПоставки = СокрЛП(СтрЗаменить(СокрЛП(Excel.Cells(Сч, 3).Value),Символы.НПП,""));
		НоваяСтр.ДатаПоставки =  СокрЛП(СтрЗаменить(СокрЛП(Excel.Cells(Сч, 4).Value),Символы.НПП,""));
		...
		КонецЕсли;
		НоваяСтр.Сумма =  СокрЛП(СтрЗаменить(СокрЛП(Excel.Cells(Сч, 12).Value),Символы.НПП,""));   //было 6
		НоваяСтр.СуммаПП =  СокрЛП(СтрЗаменить(СокрЛП(Excel.Cells(Сч, 12).Value),Символы.НПП,"")); //было 6
...
Процедура ЗагрузитьНаСервере()
	...
	Для каждого Стр Из Объект.ПлатежныеПозицииФинансирование Цикл
		Если Стр.РТУ = Документы.РеализацияТоваровУслуг.ПустаяСсылка() или
			...
			Продолжить;
		КонецЕсли;
		ЗаполнитьЗначенияСвойств(Объект.ТЧ.Добавить(),Стр);
	КонецЦикла;
	ВрТаб = Объект.ТЧ.Выгрузить();
...
Процедура СохранитьНаСервере()
	...
	Попытка
		Для Каждого СтрРеестр Из Объект.ТЧ Цикл
...
Процедура ЗагрузитьНомераПостуленийНаРС(Команда)	
	...
	ОткрытьФормуМодально("ВнешняяОбработка.УХ_ЗагрузкаРаспределенийНаРС.Форма.ФормаЗагрузкиСоответствия",Параметрыф,ЭтаФорма);
КонецПроцедуры
