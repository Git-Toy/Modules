...
Процедура ЗагрузитьФинансирование(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ФайлФинансирование) Тогда
		ТекстОшибки = НСтр("ru = 'Укажите путь к файлу финансирования!'");
	...
	ИначеЕсли Вариант = 2 Тогда
		...
		ЗагрузитьФинансированиеНаСервере();
	Иначе
		ОписаниеОповещения = Новый ОписаниеОповещения("ПослеЗагрузкиФайлаФинансированияНаСервер", ЭтотОбъект);
		НачатьПомещениеФайлаНаСервер(ОписаниеОповещения, , , , ФайлФинансирование, УникальныйИдентификатор);
	КонецЕсли;
...
Процедура ЗагрузитьОстатки(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ФайлОстатки) Тогда
		ТекстОшибки = НСтр("ru = 'Укажите путь к файлу остатков!'");
	...
	ИначеЕсли Вариант = 3 Тогда
		...
		ЗагрузитьОстаткиНаСервере();
	Иначе
		ОписаниеОповещения = Новый ОписаниеОповещения("ПослеЗагрузкиФайлаОстатковНаСервер", ЭтотОбъект);
		НачатьПомещениеФайлаНаСервер(ОписаниеОповещения, , , , ФайлФинансирование, УникальныйИдентификатор);
	КонецЕсли;
...
&НаКлиенте
Процедура ПослеЗагрузкиФайлаФинансированияНаСервер(ОписаниеПомещенногоФайла, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстОшибки = НСтр("ru = 'Загрузка данных для выбранного варианта не предусмотрена!'");
...
&НаКлиенте
Процедура ПослеЗагрузкиФайлаОстатковНаСервер(ОписаниеПомещенногоФайла, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Вариант = 3 Тогда
		ПрочитатьЛистExcel_Остатки_Сбербанк();
	ИначеЕсли Вариант = 5 Тогда
		ПрочитатьЛистExcel_Остатки_ПСБФакторинг();
	Иначе
		ТекстОшибки = НСтр("ru = 'Загрузка данных для выбранного варианта не предусмотрена!'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки);
		Возврат;
...
Процедура ПрочитатьЛистExcel_Финансирование_Сбербанк()
	...
	Для ТекущаяСтрока = ПерваяСтрока По КоличествоСтрок Цикл
		// Получим номер поставки
		НомерПоставки = ПолучитьЗначениеИзТабличногоДокумента(ТабличныйДокумент, ТекущаяСтрока, 10);
		Если НЕ ЗначениеЗаполнено(НомерПоставки) Тогда
			Продолжить;
		КонецЕсли;
		
		// Получим номер платежа
		НомерПлатежа = ПолучитьЗначениеИзТабличногоДокумента(ТабличныйДокумент, ТекущаяСтрока, 16);
		
		// Получим сумму финансирования
		СуммаФинансирования = ПолучитьЗначениеИзТабличногоДокумента(ТабличныйДокумент, ТекущаяСтрока, 14, "Число");
		Если НЕ ЗначениеЗаполнено(СуммаФинансирования) Тогда
			Продолжить;
		КонецЕсли;
		
		// Получим дату платежа
		ДатаПлатежа = ПолучитьЗначениеИзТабличногоДокумента(ТабличныйДокумент, ТекущаяСтрока, 15, "Дата");
		
		Если НЕ ЗначениеЗаполнено(ДатаПлатежа) Тогда
			ДатаПлатежа = ПолучитьЗначениеИзТабличногоДокумента(ТабличныйДокумент, ТекущаяСтрока, 11, "Дата");
		КонецЕсли;
		...
		// Получим дату поставки
		ДатаПоставки = ПолучитьЗначениеИзТабличногоДокумента(ТабличныйДокумент, ТекущаяСтрока, 11, "Дата");
		Если НЕ ЗначениеЗаполнено(ДатаПоставки) Тогда
			ДатаПоставки = ПолучитьЗначениеИзТабличногоДокумента(ТабличныйДокумент, ТекущаяСтрока, 7, "Дата");
		КонецЕсли;
		
		НоваяСтрока = Объект.ПлатежныеПозицииФинансирование.Добавить();
		НоваяСтрока.ДатаПлатежа = ДатаПлатежа;
		НоваяСтрока.ДатаПоставки = ДатаПоставки;
		НоваяСтрока.НомерПлатежа = НомерПлатежа;
		НоваяСтрока.НомерПоставки = НомерПоставки;
		НоваяСтрока.Сумма = СуммаФинансирования;
		НоваяСтрока.СуммаПП = СуммаФинансирования;
	КонецЦикла;
...
&НаСервере
Процедура ПрочитатьЛистExcel_Остатки_ПСБФакторинг()
	...
	Для ТекущаяСтрока = ПерваяСтрока По КоличествоСтрок Цикл
		...
		НоваяСтрока = Объект.ПлатежныеПозицииПеречислениеОстатков.Добавить();
		НоваяСтрока.ДатаПлатежа = ДатаПлатежа;
		НоваяСтрока.ДатаПоставки = ДатаПоставки;
		НоваяСтрока.НомерПоставки = НомерПоставки;
		НоваяСтрока.Сумма = СуммаФинансирования;
...
Процедура ПрочитатьЛистExcel_Остатки_Райффайзен()
	...
	Для ТекущаяСтрока = ПерваяСтрока По КоличествоСтрок Цикл
		...
		// Получим номер поставки
		НомерПоставки = ПолучитьЗначениеИзТабличногоДокумента(ТабличныйДокумент, ТекущаяСтрока, 13);
		Если НЕ ЗначениеЗаполнено(НомерПоставки) Тогда
			Продолжить;
		КонецЕсли;
			
		// Получим дату платежа
		ДатаПлатежа = ПолучитьЗначениеИзТабличногоДокумента(ТабличныйДокумент, ТекущаяСтрока, 23, "Дата");
	    Если Год(ДатаПлатежа) < 2000 Тогда
			ДатаПлатежа = ДобавитьМесяц(ДатаПлатежа, 2000 * 12);
		КонецЕсли;
		
		// Получим дату поставки
		ДатаПоставки = ПолучитьЗначениеИзТабличногоДокумента(ТабличныйДокумент, ТекущаяСтрока, 14, "Дата");
		
		// Получим сумму финансирования
		Сумма = ПолучитьЗначениеИзТабличногоДокумента(ТабличныйДокумент, ТекущаяСтрока, 22, "Число");
		СуммаПП = ПолучитьЗначениеИзТабличногоДокумента(ТабличныйДокумент, ТекущаяСтрока, 26, "Число");
...
Процедура ЗагрузитьФинансированиеНаСервере()
	...
	Если НЕ ЗаполнитьДокументыРТУ(Объект.ПлатежныеПозицииФинансирование) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьОстаткиНаСервере()
	...
	Если НЕ ЗаполнитьДокументыРТУ(Объект.ПлатежныеПозицииПеречислениеОстатков) Тогда
		Возврат;
	КонецЕсли;
	...
	Если Объект.ВариантЗагрузки = 5 Тогда
		...
		// Ищем один документ на один день по договору
		СтруктураОтбора = Новый Структура("СтатусЗагрузки", 0);
		ТаблицаДокументов = Объект.ПлатежныеПозицииПеречислениеОстатков.Выгрузить(
				СтруктураОтбора, "ДатаПлатежа, Дебитор, ДоговорКонтрагента, СуммаПП");
		ТаблицаДокументов.Свернуть("ДатаПлатежа, Дебитор, ДоговорКонтрагента", "СуммаПП");
		...
		Для Каждого СтрокаДокументов Из ТаблицаДокументов Цикл
			...
			Если ЗначениеЗаполнено(ПоступлениеРС) Тогда
				...
				СтрокиТЧ = Объект.ПлатежныеПозицииПеречислениеОстатков.НайтиСтроки(СтруктураОтбора);
				Для Каждого Стр Из СтрокиТЧ Цикл
					Стр.ПоступлениеРС = ПоступлениеРС;
				КонецЦикла;
			КонецЕсли;
...
&НаСервере
Функция ЗаполнитьДокументыРТУ(ТаблицаЗаполнения)
	
	// Определяем диапазон дат
	ТаблицаДат = ТаблицаЗаполнения.Выгрузить( , "ДатаПоставки");
...
Функция ПолучитьДебиторовДоговоровФакторинга()
	...
	// Мы не можем сразу взять данные настройки заполнения,
