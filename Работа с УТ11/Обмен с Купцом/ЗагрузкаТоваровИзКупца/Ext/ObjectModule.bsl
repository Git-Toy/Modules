...
Функция ЗагрузитьНоменклатурныеПозиции(Соединение, СтруктураЕдиницИзмерений, СтруктураСвойствНоменклатуры)
	...
	ТекстЗапроса =
		...
		|	ЕСТЬNULL(РеквизитыСтраныМира.Значение, ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)) КАК СтранаПроисхождения
		...
		|	ТаблицаВидовНоменклатуры.ВидНоменклатуры КАК ВидНоменклатуры,
		...
		|	ТаблицаМарокБрендов.Марка КАК Марка,
		|	ТаблицаМарокБрендов.Производитель КАК Производитель,
		...
		|	ВЫБОР
		|		КОГДА ТаблицаЗагрузки.Наименование <> ЕСТЬNULL(ТаблицаНоменклатуры.Наименование, """")
		|			ТОГДА ИСТИНА
		|		КОГДА ТаблицаЗагрузки.НаименованиеПолное <> ЕСТЬNULL(ТаблицаНоменклатуры.НаименованиеПолное, """")
		|			ТОГДА ИСТИНА
		|		КОГДА ТаблицаЗагрузки.Артикул <> ЕСТЬNULL(ТаблицаНоменклатуры.Артикул, """")
		|			ТОГДА ИСТИНА
		|		КОГДА ТаблицаЗагрузки.Вес <> ЕСТЬNULL(ТаблицаНоменклатуры.ВесЧислитель, 0)
		|			ТОГДА ИСТИНА
		|		КОГДА ЕСТЬNULL(ТаблицаРодителей.Родитель, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)) <> ЕСТЬNULL(ТаблицаНоменклатуры.Родитель, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
		|			ТОГДА ИСТИНА
		|		КОГДА ЕСТЬNULL(ТаблицаВидовНоменклатуры.ВидНоменклатуры, ЗНАЧЕНИЕ(Справочник.ВидыНоменклатуры.ПустаяСсылка)) <> ЕСТЬNULL(ТаблицаНоменклатуры.ВидНоменклатуры, ЗНАЧЕНИЕ(Справочник.ВидыНоменклатуры.ПустаяСсылка))
		|			ТОГДА ИСТИНА
		|		КОГДА ЕСТЬNULL(ТаблицаМарокБрендов.Марка, ЗНАЧЕНИЕ(Справочник.Марки.ПустаяСсылка)) <> ЕСТЬNULL(ТаблицаНоменклатуры.Марка, ЗНАЧЕНИЕ(Справочник.Марки.ПустаяСсылка))
		|			ТОГДА ИСТИНА
		|		КОГДА ЕСТЬNULL(ТаблицаМарокБрендов.Производитель, ЗНАЧЕНИЕ(Справочник.Производители.ПустаяСсылка)) <> ЕСТЬNULL(ТаблицаНоменклатуры.Производитель, ЗНАЧЕНИЕ(Справочник.Производители.ПустаяСсылка))
		|			ТОГДА ИСТИНА
		|		КОГДА ЕСТЬNULL(ТаблицаСтавокНДС.СтавкаНДС, ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.ПустаяСсылка)) <> ЕСТЬNULL(ТаблицаНоменклатуры.СтавкаНДС, ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.ПустаяСсылка))
		|			ТОГДА ИСТИНА
		|		КОГДА &ЕдиницаИзмеренияШтук <> ЕСТЬNULL(ТаблицаНоменклатуры.ЕдиницаИзмерения, ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка))
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
