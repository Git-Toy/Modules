...
Функция ЗагрузитьНоменклатурныеПозиции(Соединение, ДопСвойствоКодКупца)
	...
	ТекстЗапроса =
		...
		|	ТаблицаНоменклатуры.ware_name КАК Наименование,
		...
		|	ТаблицаНоменклатуры.unit_weight КАК Вес,
		|	ТаблицаНоменклатуры.barcode КАК Штрихкод
		...
		|	ВЫБОР
		|		КОГДА ТаблицаНоменклатуры.Вес > 0
		|			ТОГДА ЕдиницаИзмеренияВеса.Значение
		...
		|		ПО ТаблицаНоменклатуры.Штрихкод = ШтрихКоды.Штрихкод
		|			И (СправочникНоменклатура.Ссылка = ШтрихКоды.Номенклатура),
		|	Константа.ЕдиницаИзмеренияКоличестваШтук КАК ЕдиницаИзмеренияШтук,
		|	Константа.ЕдиницаИзмеренияВеса КАК ЕдиницаИзмеренияВеса
		|ГДЕ
		|	НЕ(ЕСТЬNULL(ТаблицаНоменклатуры.Наименование = СправочникНоменклатура.Наименование, ЛОЖЬ)
		|				И ЕСТЬNULL(ТаблицаНоменклатуры.Артикул = СправочникНоменклатура.Артикул, ЛОЖЬ)
		...
		|				И ЕСТЬNULL(ТаблицаНоменклатуры.Вес = СправочникНоменклатура.ВесЧислитель, ЛОЖЬ)
		|				И ЕСТЬNULL(ТаблицаНоменклатуры.Штрихкод = ШтрихКоды.Штрихкод, ЛОЖЬ))";
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(Выборка.Штрихкод) Тогда
				ЗаписатьШтрихкод(НоменклатураОбъект.Ссылка, Выборка.Штрихкод);
			КонецЕсли;
