...
&НаКлиенте
Процедура ТаблицаЗагрузкиЛицевойСчетПриИзменении(Элемент)
	
	ИзменяемаяСтрока = ЭтаФорма.Элементы.ТаблицаЗагрузки.ТекущиеДанные;
	...
	Сумма = ИзменяемаяСтрока.Сумма;
	ДатаЗагрузки = ИзменяемаяСтрока.ДатаЗагрузки;
	СтруктураРеквизитов = ПолучитьРеквизитыЛицевогоСчета(ЛицевойСчет, Сумма, ДатаЗагрузки);
...
&НаСервере
Функция ПолучитьРеквизитыЛицевогоСчета(ЛицевойСчет, Сумма, ДатаЗагрузки)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ЛицевыеСчета.Контрагент КАК Контрагент,
		|	МАКСИМУМ(ПоступлениеНаРасчетныйСчет.Ссылка) КАК ДокументЗагрузки
		|ИЗ
		|	Справочник.КУ_ЛицевыеСчета КАК ЛицевыеСчета
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеНаРасчетныйСчет КАК ПоступлениеНаРасчетныйСчет
		|		ПО ЛицевыеСчета.Ссылка = ПоступлениеНаРасчетныйСчет.КУ_ЛицевойСчет
		|			И (ПоступлениеНаРасчетныйСчет.СуммаДокумента = &Сумма)
		|			И (ПоступлениеНаРасчетныйСчет.Организация = &Организация)
		|			И (ПоступлениеНаРасчетныйСчет.СчетОрганизации = &СчетОрганизации)
		|			И (ПоступлениеНаРасчетныйСчет.Дата МЕЖДУ &ДатаЗагрузкиНачало И &ДатаЗагрузкиОкончание)
		|ГДЕ
		|	ЛицевыеСчета.Ссылка = &ЛицевойСчет
		|
		|СГРУППИРОВАТЬ ПО
		|	ЛицевыеСчета.Контрагент";
	
	Запрос.УстановитьПараметр("Сумма", Сумма);
	Запрос.УстановитьПараметр("ЛицевойСчет", ЛицевойСчет);
	Запрос.УстановитьПараметр("Организация", Объект.Организация);
	Запрос.УстановитьПараметр("СчетОрганизации", Объект.БанковскийСчет);
	Запрос.УстановитьПараметр("ДатаЗагрузкиНачало", НачалоДня(ДатаЗагрузки));
	Запрос.УстановитьПараметр("ДатаЗагрузкиОкончание", КонецДня(ДатаЗагрузки));
	Выборка = Запрос.Выполнить().Выбрать();
	
	СтруктураРезультат = Новый Структура;
	СтруктураРезультат.Вставить("Контрагент", Справочники.Контрагенты.ПустаяСсылка());
	СтруктураРезультат.Вставить("ДокументЗагрузки", Документы.ПоступлениеНаРасчетныйСчет.ПустаяСсылка());
