...
Процедура ЗаполнитьТаблицу(АдресФайла)
	...
	РазделительПолей = ";";
	КоличествоЗначенийВСтроке = 12;
	
	Для НомерЗагружаемойСтроки = 0 По КоличествоСтрокФайлаЗагрузки Цикл
		ЗагружаемаяСтрока = МассивСтрокФайлаЗагрузки.Получить(НомерЗагружаемойСтроки);
		Если ПустаяСтрока(ЗагружаемаяСтрока) Тогда
			Продолжить;
		КонецЕсли;
		
		МассивЗагружаемойСтроки = СтрРазделить(ЗагружаемаяСтрока, РазделительПолей, Истина);
		Если МассивЗагружаемойСтроки.Количество() <> КоличествоЗначенийВСтроке Тогда
			Продолжить;
		КонецЕсли;
