...
Процедура ВыгрузитьРеестрНаСервере(ПолучаемыеФайлы, ИмяФайла)
	...
	Пока ВыборкаПоЛицевымСчетам.Следующий() Цикл
		...
		// Сумма задолженности
		СуммаЗадолженности = ВыборкаПоЛицевымСчетам.Задолженность;
		СуммаЗадолженностиСтрокой = Формат(СуммаЗадолженности, "ЧДЦ=2; ЧРД=.; ЧГ=");
		МассивВыгружаемойСтроки.Добавить(СуммаЗадолженностиСтрокой);
		...
		ОбластьЛицевойСчет.Параметры.СуммаЗадолженности = СуммаЗадолженностиСтрокой;
		...
		Пока ВыборкаПоПоказателям.Следующий() Цикл
			...
			ИначеЕсли ТипПоказателя = "Услуга" Тогда
				...
				ЗначениеПоказателя = Окр(ВыборкаПоПоказателям.ЗначениеПоказателя, 2);
				ЗначениеПоказателяСтрокой = Формат(ЗначениеПоказателя, "ЧДЦ=2; ЧРД=.; ЧГ=");
				МассивПоказателей.Добавить(ЗначениеПоказателяСтрокой);
				
				ОбластьПоказатель.Параметры.СсылкаПоказателя = ВыборкаПоПоказателям.СсылкаПоказателя;
				ОбластьПоказатель.Параметры.НаименованиеПоказателя = НаименованиеУслуги;
				ОбластьПоказатель.Параметры.ТипПоказателя = ТипПоказателя;
				ОбластьПоказатель.Параметры.ЗначениеПоказателя = ЗначениеПоказателяСтрокой;
				РезультатВыгрузки.Вывести(ОбластьПоказатель);
				
				СуммаЗадолженности = СуммаЗадолженности - ЗначениеПоказателя;
			Иначе
				МассивПоказателей.Добавить("[!]");
			КонецЕсли;
		КонецЦикла;
		
		// Скорректируем ошибку округления полученную из-за распределения аванса
		Если СуммаЗадолженности <> 0 Тогда
			ЗначениеПоказателяСтрокой = Формат(ЗначениеПоказателя + СуммаЗадолженности, "ЧДЦ=2; ЧРД=.; ЧГ=");
			МассивПоказателей[МассивПоказателей.ВГраница()] = ЗначениеПоказателяСтрокой;
		КонецЕсли;
...
Функция ПолучитьДанныеРеестра()
	...
	Запрос.Текст =
		...
		|	СУММА(ВЫБОР
		|			КОГДА ТаблицаЗадолженности.Задолженность > 0
		|				ТОГДА ТаблицаЗадолженности.Задолженность
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК ЗадолженностьБезАванса,
		...
		|	ТаблицаИтоговойЗадолженности.Задолженность / ТаблицаИтоговойЗадолженности.ЗадолженностьБезАванса * ТаблицаЗадолженности.Задолженность
		|ИЗ
		|	ТаблицаЗадолженности КАК ТаблицаЗадолженности
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаИтоговойЗадолженности КАК ТаблицаИтоговойЗадолженности
		|		ПО (КУ_ЛицевыеСчета.Ссылка = ТаблицаИтоговойЗадолженности.ЛицевойСчет)
		|ГДЕ
		|	ТаблицаЗадолженности.Задолженность > 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	ТаблицаИтоговойЗадолженности.Задолженность > 0
		|
		|УПОРЯДОЧИТЬ ПО
		|	ЛицевойСчет,
