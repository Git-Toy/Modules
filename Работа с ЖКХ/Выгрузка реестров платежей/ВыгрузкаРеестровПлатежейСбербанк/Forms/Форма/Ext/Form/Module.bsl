...
&НаСервере
 Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Отчет.ДатаРеестра=КонецМесяца(ТекущаяДата());   
...
//	 ТекДок.Прочитать(Отчет.ФайлЗагрузки);
//	 ТекДок.Очистить();
//	 //ИтЗадолжн=""+Строка(Формат(Цел(Реестр.Итог("Задолженность")),"ЧЦ=15; ЧГ="))+"."+Прав(Строка(Реестр.Итог("Задолженность")),2);
//	 ИтЗадолжн=Формат(Отчет.Реестр.Итог("Задолженность"),"ЧЦ=15; ЧДЦ=2; ЧРД=.; ЧГ=");
...
//	 ТекДок.ДобавитьСтроку(НоваяСтрока);	
//	 Для каждого Стр Из Отчет.Реестр Цикл
...
//		 ТекДок.ДобавитьСтроку(НоваяСтрока);	
//	 КонецЦикла;
//	 ТекДок.Записать(Отчет.ФайлЗагрузки,КодировкаТекста.ANSI);
...
Процедура ВыгрузитьНовая(Команда)
	 ...
	 Для каждого Стр Из Отчет.Реестр Цикл
		 ...
		 НоваяСтрока = ""+Стр.НомерЛицевогоСчета+";"+Стр.ЛицевойСчетНаименование+";Сочи,"+Стр.ПомещениеНаименование+";"+Формат(Отчет.ДатаРеестра,"ДФ=MMyy")+";"+Задолжн;
		 ТекДок.ДобавитьСтроку(НоваяСтрока);	
	 КонецЦикла;
	 ТекДок.Записать(Отчет.ФайлЗагрузки,КодировкаТекста.ANSI);
	 Сообщить("Выгрузка завершена");
...
Процедура ЗаполнитьНаСервере()
	...
	ДатаК = Новый Граница(КонецДня(Отчет.ДатаРеестра), ВидГраницы.Включая);
	Запрос.УстановитьПараметр("ДатаРеестра", ДатаК);
	
	Результат = Запрос.Выполнить();
	
	ВыборкаДетальныеЗаписи = Результат.Выбрать();
	Отчет.Реестр.Очистить();
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Если Не ВыборкаДетальныеЗаписи.Задолженность = 0 Тогда
			НоваяСтрока 						= Отчет.Реестр.Добавить();
