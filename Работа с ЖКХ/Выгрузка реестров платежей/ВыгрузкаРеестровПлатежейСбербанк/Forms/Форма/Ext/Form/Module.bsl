...
Процедура ПриОткрытии(Отказ)
	...
	ЭтотОбъект.ДатаРеестра = КонецМесяца(ТекущаяДата);
...
Процедура Выгрузить(Команда)
	...
	Для Каждого СтрокаРеестра Из Объект.Реестр Цикл
		...
		МассивСтроки.Добавить(СтрокаРеестра.ЕдиныйЛицевойСчет);
		МассивСтроки.Добавить(СтрокаРеестра.КодДомаПоФИАС);
...
Процедура ЗаполнитьРеестрНаСервере()
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(КУ_ПомещенияЖилогоДома.НомерКвартиры, 0) КАК НомерКвартиры,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НомерКвартиры > 0 Тогда
			НоваяСтрока.КодДомаПоФИАС = НоваяСтрока.КодДомаПоФИАС + "," + Формат(НомерКвартиры, "ЧГ=");
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьРеквизитыЛицевогоСчета(ЛицевойСчет)
	...
	СтруктураРезультат.Вставить("ЕдиныйЛицевойСчет", "");
	СтруктураРезультат.Вставить("КодДомаПоФИАС", "");
	...
	Если Выборка.Следующий() Тогда
		...
		НомерКвартиры = Выборка.НомерКвартиры;
		Если НомерКвартиры > 0 Тогда
			СтруктураРезультат.КодДомаПоФИАС = СтруктураРезультат.КодДомаПоФИАС + "," + Формат(НомерКвартиры, "ЧГ=");
		КонецЕсли;
	КонецЕсли;
